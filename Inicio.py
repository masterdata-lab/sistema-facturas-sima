...
# 1. SETUP INICIAL DE LA PÁGINA (Debe ser la primera línea)
st.set_page_config(page_title="DPA | ERP", page_icon="🏢", layout="wide", initial_sidebar_state="collapsed")

# 2. DEFINIR EL CONTENIDO DEL TABLERO COMO UNA FUNCIÓN
# Esto evita el error de recursión (bucle infinito)
def tablero_principal():
    ...
    with col1:
        with st.container(border=True):
            ...
            st.markdown("Subí facturas y comprobantes desde tu PC o celular.")
            st.write("")
    ...
    with col3:
        with st.container(border=True):
            ...
            st.markdown("Revisión humana, asignación de gerencias y validación.")
...
pg = st.navigation({
    ...
    "🧾 Facturación": [
        ...
        st.Page("pages/4_Buscador.py", title="Buscador y Tableros", icon="🔍") # Movido adentro de Facturación
    ],
    "🔧 Mantenimiento": [
        # st.Page("pages/5_Taller.py", title="Gestión de OT Taller", icon="⚙️")
    ]
})
